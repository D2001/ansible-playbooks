...
def render(ipv6=False, boot=False):
    ...
    if ipv6:
        lines += ['-A INPUT -i eth1 -s fe80::/10 -p udp --sport 547 --dport 546 -j ACCEPT']
    else:
        lines += ['-A INPUT -i eth1 -s 192.168.0.1 -p udp --sport 67 --dport 68 -j ACCEPT',
                  '-A INPUT -i virbr0 -p udp -m multiport --dports 53,67 -j ACCEPT',
                  '-A INPUT -i virbr0 -p tcp --dport 53 -j ACCEPT']
    lines += [f'-A INPUT -i eth1 -s {lan} -p tcp -m multiport --dports 22,1880,3000,3080,8000,8123,18555 -j ACCEPT',
              f'-A INPUT -i eth1 -s {lan} -p udp -m multiport --dports 1900,5353 -j ACCEPT',
              f'-A INPUT -i eth1 -s {lan} -p udp -m multiport --sports 1900,5353 -j ACCEPT']
    if not ipv6:
    ...
    for bridge in ('docker0', 'br+'):
        lines += [f'-A DOCKER-USER -i eth1 -o {bridge} -s {lan} -p tcp -m multiport --dports 1880,8000 -j RETURN']
        if not ipv6:
